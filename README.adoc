...
You will build a server that accepts a message that carries a message's name. In response,
the server will push a wallResponse into a queue to which the client is subscribed.
...
creating a wallResponse and publishing that wallResponse on a separate queue to which the client
is subscribed. The wallResponse will also be a JSON object, which as the following listing
shows:
...
To model the wallResponse representation, add another plain old Java object with a `content`
...
Next, you will create a controller to receive the hello message and send a wallResponse
message.
...
the `/hello` destination, the `wallResponse()` method is called.
...
After the one-second delay, the `wallResponse()` method creates a `Greeting` object and
...
wallResponse messages back to the client on destinations prefixed with `/topic`. It also
...
example, `/app/hello` is the endpoint that the `GreetingController.wallResponse()` method is
mapped to handle.
...
destination, where the server will publish wallResponse messages. When a wallResponse is received
on that destination, it will append a paragraph element to the DOM to display the wallResponse
message.

The `sendName()` function retrieves the name entered by the message and uses the STOMP client
to send it to the `/app/hello` destination (where `GreetingController.wallResponse()` will
receive it).
...
delay, the server sends a message back with a "`Hello`" wallResponse that is displayed on the
